...
`workloads.go` resolves each service's image from its own key
(`imageFor("ingestion-api", cfg.imageTagIngestionApi)` /
`imageFor("consumer-worker", cfg.imageTagConsumerWorker)`) — a `pulumi up`
triggered by one service's pipeline only changes that service's Helm values.
...
- **Consumer spans** (`internal/adapter/messaging/consumer.go`) carry
  `payment_method`, `payment_queue`, `routing_key` (the topic-exchange
  binding key the message arrived on, e.g. `payment.pix`), `retry_count`,
  and `is_retry` — alongside the existing `message_id`/`redelivered`. Every
  span is per-message, so a single trace shows exactly which method/queue/
  binding it belongs to and whether this delivery was a retry.
- **`consumer.messages_processed_total`** (counter) — incremented once per
  message handled, tagged `payment_method`/`payment_queue`/`routing_key`/
  `outcome` (`ack` | `retry_scheduled` | `poison_dlq`). Lets a panel show
  per-method throughput and poison rate side by side.
- **`consumer.retry_attempts_total`** (counter) — incremented only when a
  delivery's `retry_count > 0`, tagged the same dimensions plus `retry_count`
  itself (the *count value* of that specific attempt, not a running total) —
  this is what answers "how many retries is method X generating, and how
  deep." Kept as a separate counter from `messages_processed_total` rather
  than folding `retry_count` into every measurement's tags, to avoid tagging
  every successful first-attempt message with a `retry_count=0` dimension
  that adds cardinality for no signal.
- **`outbox.published_total{method=...}`** (publisher side,
  `internal/usecase/outbox/dispatch.go`) — incremented per-message (not
  batched) so it carries a `method` tag, feeding capacity planning for the
...
> **Status: scripts written, `loadtest/` exists — not yet run against a real
> target.** `k6-baseline.js` and `k6-autoscale.js` were validated with
> `k6 inspect` (parses cleanly, no syntax errors). `k6-consumer.js` was
> checked the same way and correctly resolves its `k6/x/amqp`/`k6/x/sql`
> dependencies (proving the import syntax is valid) but its actual API calls
> against those xk6 extensions are an unverified best-effort sketch — the
> custom binary (`make k6-ext-build`) was never built or run. None of the
> three has been run against a live `ingestion-api`/RabbitMQ/Postgres, so
> the threshold values and the consumer script's extension API calls are
> both still placeholders to calibrate/verify on a real run.
