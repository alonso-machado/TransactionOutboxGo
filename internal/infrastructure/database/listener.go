...
// Listener is a dedicated Postgres connection that LISTENs on a given
// channel and pushes a value onto Notify for every NOTIFY it receives, so
// DispatchOutbox can wake up immediately on enqueue instead of waiting out
// the poll interval. outbox-worker uses one Listener on "order_outbox_new"
// for the order dispatcher; payment_event_outbox has no Listener (poll-only
// — lower volume, doesn't need the low-latency path).
//
...
// NewListener creates a Listener bound to dsn, listening on channel. Call
// Run in a goroutine to start listening; Notify receives a value
// (non-blocking best-effort) for every notification.
