...
// Path #9: PUBLISHED rows older than the prune window must be removed by
...
// an hour in a test.
func TestOutboxPruning_RemovesOldPublishedRowsOnly(t *testing.T) {
	truncateAll(t)

	// Ingest and publish a row, then backdate it to look old.
	body, headers := pixBody("evt-prune-old-1", "prov-prune-old-1", "")
	...
	require.NoError(t, suite.db.Model(&persistence.OutboxMessageModel{}).
	...
	body2, headers2 := pixBody("evt-prune-recent-1", "prov-prune-recent-1", "")
	...
	var remaining []persistence.OutboxMessageModel
	require.NoError(t, suite.db.Find(&remaining).Error)
